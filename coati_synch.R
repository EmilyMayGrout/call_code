#Coati time synch --  written by Ari
#Script to synchronize sorokas on coatis to UTC time
#This script produces an offset value for each date / soroka ID combo, based on a linear fit to the synch calls (fireworks)
#The set of synch calls comes from an input file, where the time in file and time in UTC of each synch call are noted.
#The first firework in the morning and afternoon are used, if present. Others are not used.
...
#Because of this, the adjusted times will probably be accurate to within a few sec

#----PARAMETERS---
utc_offset <- -5 #local time offset from UTC, in hours - should be negative 5 for panama
path_to_synch_file <- 'C:/Users/egrout/Dropbox/coaticalls/processed/synch_01.03.24.csv' #where the synch table is located
